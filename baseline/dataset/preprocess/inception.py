...
def preprocess_for_train(image_bytes, use_bfloat16, image_size, img_augment, min_area=0.08):
...
def preprocess_image(image_bytes, is_training=False, use_bfloat16=False,
                     image_size=224, img_augment=None, min_area=0.08):
...
def center_crop(image, size=224):
    ...
    padded_center_crop_size = tf.cast(((size / (size + CROP_PADDING)) * tf.cast(tf.minimum(image_height, image_width), tf.float32)), tf.int32)
...
def distort_crop(image, size=224, aspect_ratio_range=(3. / 4., 4. / 3.), area_range=(0.08, 1.0), min_object_covered=0.1, max_attempts=10, scope=None):
    original_shape = tf.shape(image)
    bbox = tf.constant([0.0, 0.0, 1.0, 1.0], dtype=tf.float32, shape=[1, 1, 4])
